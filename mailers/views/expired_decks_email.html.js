module.exports = (name, expiredDecks, url) => {
  ...
  if (expiredDecks.length > 2) {
    headline = `Your decks <strong>${expiredDecks[0]
      .title}</strong> and <strong>${expiredDecks.length -
      1} others</strong> are due to be reviewed. Their recall strength in Pensieve just dropped below 50%, which means they are about to be forgotten.`;
  } else if (expiredDecks.length === 2) {
    headline = `Your decks <strong>${expiredDecks[0].title}</strong> and <strong>${expiredDecks[1]
      .title}</strong> are due to be reviewed. Their recall strength in Pensieve just dropped below 50%, which means they are about to be forgotten.`;
  } else {
    headline = `Your deck <strong>${expiredDecks[0]
      .title}</strong> is due to be reviewed. Its recall strength in Pensieve just dropped below 50%, which means it is about to be forgotten.`;
  }
                ...
                <table align="left" border="0" cellpadding="0" cellspacing="0" style="max-width:100%;min-width:100%;border-collapse:collapse" width="100%" >
                  <tbody><tr>
                    ...
                    <td valign="top"  style="padding-top:0;padding-right:18px;padding-bottom:9px;padding-left:18px;word-break:break-word;color:#202020;font-family:Helvetica;font-size:16px;line-height:150%;text-align:left">
                      ...
                      <p style="margin:20px 0;padding:0;color:#202020;font-family:Helvetica;font-size:16px;line-height:150%;text-align:left">${headline} If you want to improve your retention of those notes, you should review them now.</p>
